...
def my_3comp_data_generator_b(batch_size,x_data,n_data,dts,sig_inds,noise_inds,sr,std,valid=False):
    ...
    while True:
        ...
        noiseinds=noise_inds[start_of_noise_batch:start_of_noise_batch+batch_size//2]
        #length of each component
        nlen=x_data['waves'].shape[1]//3
        # grab batch
        comp1=np.concatenate((x_data['waves'][list(datainds),:nlen],n_data['waves'][list(noiseinds),:nlen]))
        comp2=np.concatenate((x_data['waves'][list(datainds),nlen:2*nlen],n_data['waves'][list(noiseinds),nlen:2*nlen]))
        comp3=np.concatenate((x_data['waves'][list(datainds),2*nlen:],n_data['waves'][list(noiseinds),2*nlen:]))
        # make target data vector for batch
        ...
        for ii, targ in enumerate(target):
            ...
            if targ==0:
                ...
                tmpshift=np.concatenate((tmp[int(targ*sr):],np.zeros(int(targ*sr))))
                # add p and s targets                        
                batch_target[ii,:]=signal.gaussian(nlen,std=int(std*sr))-tmpshift
