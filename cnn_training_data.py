...
# set upts
sr=100 # sample rate for datad
winsize=30 # seconds

# make templates
for myphases in ['P']: #,'S','N']:
    stall=np.array([]).reshape(0,3*(winsize*sr+1))
    ...
    while len(stall) < 250000:
        if ii == len(inds):
            ...
            checkphaseinfo=download_tools.check_phase_info(evdf)
            if checkphaseinfo:  
                stout=download_tools.make_training_data_3comp(evdf,sr,winsize,myphases)      
                stall=np.vstack([stall, stout])
                print(str(stall.shape[0])+" TOTAL TRACES", flush=True)
            ii+=1
    
# #    # plot the data
# #    download_tools.plot_training_data(stall,sr,porsall,winsize)
    
    # save the data
    with open('pnsn_ncedc_3comp_'+myphases+'_100_training_data.pkl', 'wb') as f:
        pickle.dump(stall, f)